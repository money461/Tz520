...
$(function(){
	...
	$('.submit_data').click(function(){
		...
		layer.confirm('确认数据检查无误并提交?',function(index){
			$.ajax({
		        type: "POST",
		        url: '/tz/admin/tzContentCategory/addOrUpdate',
		        data: {id:id,name:name,logoUrl:logoUrl,linkAddress:linkAddress,sort:sort,type:"update"},
		        dataType: "json",
		        beforeSend: function () {
		            //防止重复提交
...
function getDate(){
	$.ajax({
        type: "post",
        url: '/tz/admin/tzContentCategory/selectContentCategoryById',
        data: {id:id},
        dataType: "json",
        success: function(data){
            if(data.flag){
