...
var init = {
 getList:function(curPage,rows) {
	 ...
	 var userPhone = $("#userPhone").val(); //用户账户电话
	 var consignName = $("#consignName").val(); //收货人
    ...
    $.ajax({
        ...
        data: {"rows": rows, "curPage": curPage,id:id,userPhone:userPhone,consignName:consignName,status:status,startTime:startTime,endTime:endTime},
...
//	        console.info(list);
            ...
            if (list != null && list.length > 0) {
                for (var x = 0; x < list.length; x++) {
                	...
                	//其他字段若为空，则不显示null
                	var payment = list[x].payment;
                	var orderNum = list[x].orderNum;
                	...
                	if(paymentType==1){
         			...
         			if(status == 0){
         				...
         				span3='<a title="编辑物流" href="javascript:;" onclick="delivery_edit(\'编辑物流\',\''+list[x].id+'\')" style="text-decoration:none"><span class="label label-primary radius">编辑物流</span></a>'
         				       +'</br><a title="退款" href="javascript:;" onclick="drawback(this,\''+list[x].id+'\')"  style="text-decoration:none"><span class="label label-primary radius">退款</span></a>';
    				...
    				'<td id="shipping_'+list[x].id+'\">'+span4+'</td>'+
    				'<td>'+isNull(list[x].userPhone)+'</td>'+
    				'<td>'+isNull(list[x].consignName)+'</td>'+
    				'<td class="status_ql">'+span+'</td>'+
    				'<td>'+isNull(payment)+'</td>'+
    				'<td>'+isNull(orderNum)+'</td>'+
    				'<td>'+span2+'</td>'+
...
function computeOrder(){
	 ...
	 $.ajax({
		    type:'post',
			url:'/tz/admin/order/computeOrder',
			data: {id:id,userPhone:userPhone,consignName:consignName,status:status,startTime:startTime,endTime:endTime},
			dataType:'json',
			success: function (data) {
				if(data.flag){
					var map = data.data;
					if($.isEmptyObject(map)){  //判断数据是否null
						$(".totalPayment").html('');
						$(".totalItemNum").html('');
					}else{
						var totalPayment = map["totalPayment"];
						var totalItemNum = map["totalItemNum"];
						$(".totalPayment").html('订单总金额 : <strong style="color:red">'+totalPayment+'</strong>元&nbsp;&nbsp;');
						$(".totalItemNum").html('商品总数量 : <strong style="color:red">'+totalItemNum+'</strong>件&nbsp;&nbsp;');
					}
				}else{
					layer.msg('统计失败，请稍后重试！',{icon:5,time:1000});
				}
			},
			error:function(data) {
