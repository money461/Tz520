...
var init = {
 getList:function(curPage,rows) {
    ...
    $.ajax({
        ...
        url: "/tz/admin/userLoveDetails/list",//后端处理数据，返回json格式
        data: {"rows":rows, "curPage":curPage,startTime:startTime,endTime:endTime,
        	phone:phone,userName:userName,type:type},
            ...
            if (list != null && list.length > 0) {
                ...
                for (var x = 0; x < list.length; x++) {
                	...
                	showTypeSpan="";
         			...
         			showType = list[x].showType;
         			//支付类型
         			if(showType == 0){
         				showTypeSpan='<i class="Hui-iconfont">&#xe71f;</i>（支付宝）';
         			}else if(showType == 1){
         				showTypeSpan='<i class="Hui-iconfont">&#xe719;</i>（微信）';
         			}else{
         				showTypeSpan='';
         			}
         			html += '<tr>'+
    				...
    				'<td>'+showTypeSpan+'</td>'+
    				'<td>'+isNull(list[x].showAccount)+'</td>'+
