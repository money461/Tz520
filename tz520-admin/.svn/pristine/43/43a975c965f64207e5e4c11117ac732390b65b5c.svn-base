...
var init = {
 getList:function(curPage,rows) {
    ...
    $.ajax({
        ...
        data: {"rows": rows, "curPage": curPage,startTime:startTime,endTime:endTime,name:name},
        ...
        success: function (data) {
            ...
            if (list != null && list.length > 0) {
                for (var x = 0; x < list.length; x++) {
         			var span = "";//权限信息
         			var authFunctionList = list[x].authFunctionList;
         			for(var k=0; k<authFunctionList.length;k++){
         				span +="<span>"+authFunctionList[k].name+"</span></br>";
         			}
