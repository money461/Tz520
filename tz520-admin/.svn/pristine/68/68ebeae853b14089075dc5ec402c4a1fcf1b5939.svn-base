...
var init = {
 getList:function(curPage,rows) {
    ...
    $.ajax({
        ...
        url: "/tz/admin/tzItemCategory/list",//后端处理数据，返回json格式
        ...
        success: function (data) {
            ...
            if (list != null && list.length > 0) {
                for (var x = 0; x < list.length; x++) {
                	...
                	span='<a title="编辑" href="javascript:;" onclick="tzItemCategory_edit(\'编辑商品分类\',\''+list[x].id+'\')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>'
				       +'<a title="删除" href="javascript:;" onclick="tzItemCategory_del(this,\''+list[x].id+'\')" class="ml-5" style="text-decoration:none"><i id="del" class="Hui-iconfont">&#xe6e2;</i></a>';
         			html += '<tr>'+
    				'<td>'+list[x].id+'</td>'+		
    				'<td>'+list[x].name+'</td>'+
    				'<td>'+logo+'</td>'+
...
/*商品分类-增加*/
function tzItemCategory_add(title,url,w,h){
	layer_show(title,url,w,h);
}
/*商品分类-编辑*/
function tzItemCategory_edit(title,id,w,h){
	url =  "/tz/admin/tzItemCategory/addOrUpdatePage?type=update&id="+id;
	layer_show(title,url,w,h);
}
/*用户-查看
...
/*商品分类-删除*/
function tzItemCategory_del(obj,id){
 	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '/tz/admin/tzItemCategory/deleteItemCategoryById',
			data: {id:id},
			dataType: 'json',
			success: function(data){
	            if(data.flag){
