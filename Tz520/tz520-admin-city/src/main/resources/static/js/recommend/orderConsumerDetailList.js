...
var init = {
 getList:function(curPage,rows) {
    ...
    $.ajax({
        ...
        url: "/tz/admin/recommend/orderConsumerDetialList",//后端处理数据，返回json格式
        data: {"rows":rows, "curPage":curPage,orderId:orderId},
        ...
        success: function (data) {
            ...
            if (list != null && list.length > 0) {
            	 for (var x = 0; x < list.length; x++) {
         			html += '<tr>'+
    				'<td>'+isNull(list[x].orderId)+'</td>'+		
    				'<td>'+isNull(list[x].itemTitle)+'</td>'+
    				'<td><img width="210" class="picture-thumb" src="'+isNull(list[x].homepageUrl)+'"></td>'+		
    				'<td>'+isNull(list[x].num)+'</td>'+
    				'<td>'+list[x].totalFee+'</td>'+
